...
class Cart extends Model
{
    ...
    public function removeProduct(Products $product, $all = false)
    {
        $sql = new Sql();
        if ($all) {
            $sql->select("UPDATE tb_cartsproducts SET dtremoved = NOW() WHERE idcart = :idcart AND idproduct = :idproduct  AND dtremoved IS NULL", [
                ':idcart' => $this->getidcart(),
                ':idproduct' => $product->getidproduct(),
            ]);
        } else {
            $sql->select("UPDATE tb_cartsproducts SET dtremoved = NOW() WHERE idcart = :idcart AND idproduct = :idproduct AND dtremoved IS NULL LIMIT 1", [
                ':idcart' => $this->getidcart(),
                ':idproduct' => $product->getidproduct(),
            ]);
        }
    ...
    public function getProducts()
    {
        ...
        return Products::checkList($rows); //Trata de incluir as imagens no produto.
    }
